...
for n_line, line in enumerate(program):
    line = line.replace('\n', '')
    if not line:
        out.append(0x00)
        out.append(0x00)
    elif re.match(':[a-z]+', line):
        jump_point = line.replace(':', '')
        jump_points[jump_point] = pc + n_line * 2
        out.append(0x00)
        out.append(0x00)
    elif line == 'CLS':
        out.append(0x00)
    ...
    elif re.match('JP [a-z]+', line): # modified
        jump_point = line.replace('JP ', '')
        index = jump_points[jump_point]
        opcode = 0x1000 + index
        out.append((opcode & 0xFF00) >> 8)
        out.append((opcode & 0x00FF))
    elif re.match('CALL [a-z]+', line): # modified
        jump_point = line.replace('CALL ', '')
        index = jump_points[jump_point]
        opcode = 0x2000 + index
        out.append((opcode & 0xFF00) >> 8)
        out.append((opcode & 0x00FF))
